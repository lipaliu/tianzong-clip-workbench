...
test("ships product metadata and removes the disposable starter preview", async () => {
  ...
  assert.match(styles, /\.model-gallery-track[\s\S]*?grid-auto-flow: column[\s\S]*?grid-auto-columns: clamp\(196px, 19vw, 236px\)[\s\S]*?animation: model-gallery-marquee 52s linear infinite/);
  ...
  assert.match(styles, /\.model-gallery-track img[\s\S]*?width: 100%[\s\S]*?height: 100%[\s\S]*?opacity: 1[\s\S]*?filter: none/);
