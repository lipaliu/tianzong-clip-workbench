...
test("server-renders the Tianzong project workbench", async () => {
  ...
  assert.equal(html.match(/class="model-gallery-photo"/g)?.length, 18);
  assert.equal(html.match(/data-gallery-cycle="0"/g)?.length, 9);
  assert.equal(html.match(/data-gallery-cycle="1"/g)?.length, 9);
  assert.match(html, /自动播放 · 悬停或触摸暂停 · 可左右滑动/);
...
test("ships product metadata and removes the disposable starter preview", async () => {
  ...
  assert.match(page, /window\.requestAnimationFrame\(advanceGallery\)/);
  assert.match(page, /prefers-reduced-motion: reduce/);
  assert.match(page, /repeatedFirstPhoto\.offsetLeft - firstPhoto\.offsetLeft/);
  assert.match(page, /onTouchStart=\{pauseGalleryForTouch\}/);
  assert.match(page, /onScroll=\{handleGalleryScroll\}/);
  assert.match(page, /onLostPointerCapture=\{stopGalleryDrag\}/);
  assert.match(page, /onMouseEnter=\{\(\) => setGalleryHovered\(true\)\}/);
  assert.match(page, /galleryCycleWidthRef/);
  assert.match(page, /new ResizeObserver\(measureCycle\)/);
  assert.match(page, /new IntersectionObserver/);
  assert.match(page, /aria-pressed=\{galleryUserPaused\}/);
  assert.match(page, /继续播放/);
  ...
  assert.match(styles, /\.model-mini-gallery[\s\S]*?grid-auto-flow: column[\s\S]*?grid-auto-columns: clamp\(196px, 19vw, 236px\)[\s\S]*?overflow-x: auto[\s\S]*?scroll-snap-type: x proximity/);
  assert.match(styles, /\.model-mini-gallery\.auto-scrolling[\s\S]*?scroll-snap-type: none/);
  assert.match(styles, /\.model-gallery-toggle[\s\S]*?border-radius: 999px/);
